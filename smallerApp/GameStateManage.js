...
import { parsePGN, expandVariations } from "./Util/ChessNavigator2.js";
...
export class GameStateManager {
    ...
    computerMove() {
        if (this.mode_type !== "computer_mode") return;
        console.log(this.TAG + `Computer move...`)
        ...
        this.analyse(fen).then((result) => {
            ...
            if (this.mode_type === "practice_mode") {
                // get next move... 
    ...
    onStart() {
        ...
        if (this.player === 'black' && this.mode_type === "computer_mode") {
    ...
    //             let movesCoords = []
    //             moves.map((sqr) => { movesCoords.push(sqr.position) });
